...
function nexus-groovy {
    local source_filename="./$1.groovy"

    local delete_result=$(http \
        -a "$admin_username:$admin_password" \
        --ignore-stdin \
        DELETE http://$nexus_domain/service/rest/v1/script/provision.groovy)   

    local create_result=$(http \
        -a "$admin_username:$admin_password" \
        --ignore-stdin \
        --check-status \
        POST http://$nexus_domain/service/rest/v1/script \
    ...
    http \
        ...
        --ignore-stdin \
        --check-status \
        POST http://$nexus_domain/service/rest/v1/script/provision.groovy/run \
